...
async function listAccounts() {
    ...
    try {
        ...
        if (tokens.access_token || tokens.refresh_token) {
            log(
                `  ${colorize('yellow', '⚠️  Old token format detected. Will be migrated on next auth.')}`,
            );
            ...
            const status =
                hasAccessToken && hasRefreshToken && !isExpired
            ...
            log(
                `  ${colorize('cyan', 'normal'.padEnd(10))} ${status} (legacy format)`,
            );
            return;
        }
        ...
        for (const account of accounts) {
            ...
            const isExpired = tokenInfo.expiry_date
                ? Date.now() >= tokenInfo.expiry_date
                : true;
...
async function authenticateAccount(accountMode) {
    ...
    try {
        ...
        error(
            `Failed to authenticate ${accountMode} account: ${error.message}`,
        );
        process.exit(1);
    }
}
...
function showUsage() {
    ...
    log(
        '  node scripts/account-manager.js status        # Check account status',
    );
    log(`\n${colorize('bright', 'Environment Variables:')}`);
    log(
        '  GOOGLE_ACCOUNT_MODE     Set to "test" or "normal" (default: normal)',
    );
