...
export const ToolSchemas = {
    ...
    'search-events': z.object({
        calendarId: z
            .string()
            .describe(
                "ID of the calendar (use 'primary' for the main calendar)",
            ),
        query: z
            .string()
            .describe(
    ...
    'create-event': z.object({
        ...
        description: z
            .string()
            .optional()
            .describe('Description/notes for the event'),
        start: z
            .string()
            .refine((val) => {
                const withTimezone =
        ...
        reminders: z
            .object({
                useDefault: z
                    .boolean()
                    .describe('Whether to use the default reminders'),
                overrides: z
                    .array(
                        z
                            .object({
    ...
    'update-event': z
        .object({
            ...
            description: z
                .string()
                .optional()
                .describe('Updated description/notes'),
            start: z
                .string()
                .refine((val) => {
                    const withTimezone =
            ...
            originalStartTime: z
                ...
                .describe(
                    "Original start time in the ISO 8601 format '2024-01-01T10:00:00'",
                )
                .optional(),
            futureStartDate: z
                .string()
                ...
                .describe(
                    "Start date for future instances in the ISO 8601 format '2024-01-01T10:00:00'",
                )
                .optional(),
        })
        .refine(
            (data) => {
                ...
                if (
                    data.modificationScope === 'thisEventOnly' &&
        ...
        .refine(
            (data) => {
                ...
                if (
                    data.modificationScope === 'thisAndFollowing' &&
                    !data.futureStartDate
                ) {
                    return false;
                }
                return true;
...
export class ToolRegistry {
    private static extractSchemaShape(schema: z.ZodType<any>): any {
        ...
        if (schemaAny._def && schemaAny._def.typeName === 'ZodEffects') {
            return ToolRegistry.extractSchemaShape(schemaAny._def.schema);
        }
    ...
    private static tools: ToolDefinition[] = [
        ...
        },
        {
            ...
            handlerFunction: async (
                args: ListEventsInput & { calendarId: string | string[] },
            ) => {
                ...
                if (
                    ...
                    try {
                        ...
                        if (
                            ...
                            if (parsed.length > 50) {
                                throw new Error(
                                    'Maximum 50 calendars allowed per request',
                                );
                            }
                            ...
                            throw new Error(
                                'JSON string must contain an array of non-empty strings',
                            );
                        }
                    } catch (error) {
                        throw new Error(
                ...
                if (Array.isArray(processedCalendarId)) {
                    ...
                    if (
                        !processedCalendarId.every(
                            (id) => typeof id === 'string' && id.length > 0,
                        )
                    ) {
                        throw new Error(
                            'All calendar IDs must be non-empty strings',
                        );
                    }
                    if (
                        new Set(processedCalendarId).size !==
                        processedCalendarId.length
                    ) {
                        throw new Error(
                            'Duplicate calendar IDs are not allowed',
                        );
    ...
    static async registerAll(
        ...
        for (const tool of ToolRegistry.tools) {
            ...
            server.registerTool(
                ...
                {
                    description: tool.description,
                    inputSchema: ToolRegistry.extractSchemaShape(tool.schema),
                },
                async (args: any) => {
