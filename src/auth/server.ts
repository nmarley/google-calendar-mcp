    ...
    private createServer(): http.Server {
        const server = http.createServer(async (req, res) => {
            ...
            if (url.pathname === '/') {
                ...
                const clientForUrl =
                    this.flowOAuth2Client || this.baseOAuth2Client;
        ...
        `);
                ...
                try {
                    const { tokens } =
                        await this.flowOAuth2Client.getToken(code);
            ...
            new Promise<boolean>((_, reject) => {
                setTimeout(
                    () =>
                        reject(
                            new Error(
                                'Auth server start timed out after 10 seconds',
                            ),
                        ),
                    10000,
                );
        ...
        return new Promise((resolve, reject) => {
            if (this.server) {
                ...
                const timeout = setTimeout(() => {
                    process.stderr.write(
                        'Server close timeout, forcing exit...\n',
                    );
                    this.server = null;
                    resolve();
