...
export class TokenManager {
    ...
    private async saveMultiAccountTokens(
        multiAccountTokens: MultiAccountTokens,
    ): Promise<void> {
        await this.ensureTokenDirectoryExists();
        await fs.writeFile(
            this.tokenPath,
            JSON.stringify(multiAccountTokens, null, 2),
            {
                mode: 0o600,
            },
        );
    ...
    private setupTokenRefresh(): void {
        this.oauth2Client.on('tokens', async (newTokens) => {
            try {
                ...
                const currentTokens =
                    multiAccountTokens[this.accountMode] || {};
                ...
                if (process.env.NODE_ENV !== 'test') {
                    process.stderr.write(
                        `Tokens updated and saved for ${this.accountMode} account\n`,
                    );
                }
            } catch (error: unknown) {
                ...
                if (
                    ...
                    try {
                        ...
                        if (process.env.NODE_ENV !== 'test') {
                            process.stderr.write(
                                `New tokens saved for ${this.accountMode} account\n`,
                            );
                        }
                    } catch (writeError) {
    ...
    private async migrateLegacyTokens(): Promise<boolean> {
        ...
        try {
            ...
            if (!legacyTokens || typeof legacyTokens !== 'object') {
                process.stderr.write(
                    'Invalid legacy token format, skipping migration\n',
                );
                return false;
            }
            ...
            try {
                ...
                process.stderr.write(
                    `Warning: Could not remove legacy token file: ${unlinkErr}\n`,
                );
            }
    ...
    async loadSavedTokens(): Promise<boolean> {
        try {
            ...
            if (!tokenExists) {
                ...
                if (!migrated) {
                    process.stderr.write(
                        `No token file found at: ${this.tokenPath}\n`,
                    );
                    return false;
                }
            }
            ...
            if (!tokens || typeof tokens !== 'object') {
                process.stderr.write(
                    `No tokens found for ${this.accountMode} account in file: ${this.tokenPath}\n`,
                );
                return false;
            }
            ...
            process.stderr.write(
                `Loaded tokens for ${this.accountMode} account\n`,
            );
            return true;
        } catch (error: unknown) {
            process.stderr.write(
                `Error loading tokens for ${this.accountMode} account: `,
            );
            if (
                error instanceof Error &&
                'code' in error &&
                ...
                try {
                    ...
                    process.stderr.write(
                        'Removed potentially corrupted token file\n',
                    );
                } catch (unlinkErr) {
                    /* ignore */
                }
    ...
    async refreshTokensIfNeeded(): Promise<boolean> {
        ...
        if (isExpired && this.oauth2Client.credentials.refresh_token) {
            if (process.env.NODE_ENV !== 'test') {
                process.stderr.write(
                    `Auth token expired or nearing expiry for ${this.accountMode} account, refreshing...\n`,
                );
            }
            try {
                ...
                if (process.env.NODE_ENV !== 'test') {
                    process.stderr.write(
                        `Token refreshed successfully for ${this.accountMode} account\n`,
                    );
                }
                return true;
            } catch (refreshError) {
                if (
                    ...
                    process.stderr.write(
                        `Error refreshing auth token for ${this.accountMode} account: Invalid grant. Token likely expired or revoked. Please re-authenticate.\n`,
                    );
                    return false; // Indicate failure due to invalid grant
                }
                // Handle other refresh errors
                process.stderr.write(
                    `Error refreshing auth token for ${this.accountMode} account: `,
                );
            ...
            process.stderr.write(
                `No access or refresh token available for ${this.accountMode} account. Please re-authenticate.\n`,
            );
            return false;
        } else {
    ...
    async saveTokens(tokens: Credentials): Promise<void> {
        try {
            ...
            this.oauth2Client.setCredentials(tokens);
            process.stderr.write(
                `Tokens saved successfully for ${this.accountMode} account to: ${this.tokenPath}\n`,
            );
        } catch (error: unknown) {
            process.stderr.write(
                `Error saving tokens for ${this.accountMode} account: ${error}\n`,
            );
            throw error;
        }
    }
    ...
    async clearTokens(): Promise<void> {
        try {
            ...
            if (Object.keys(multiAccountTokens).length === 0) {
                ...
                await this.saveMultiAccountTokens(multiAccountTokens);
                process.stderr.write(
                    `Tokens cleared for ${this.accountMode} account\n`,
                );
            }
        } catch (error: unknown) {
            if (
                ...
                process.stderr.write(
                    `Error clearing tokens for ${this.accountMode} account: ${error}\n`,
                );
