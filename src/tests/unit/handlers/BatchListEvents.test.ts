...
describe('Batch List Events Functionality', () => {
    ...
    beforeEach(() => {
        ...
        vi.spyOn(listEventsHandler as any, 'getCalendar').mockReturnValue(
            mockCalendarApi,
        );
    });
    ...
    describe('Input Validation', () => {
        ...
        it('should validate array of calendar IDs', () => {
            ...
            const input = {
                calendarId:
                    '["primary", "work@example.com", "personal@example.com"]',
                timeMin: '2024-01-01T00:00:00Z',
            };
            ...
            expect(result.data?.calendarId).toBe(
                '["primary", "work@example.com", "personal@example.com"]',
            );
        });
    ...
    describe('Single Calendar Events (Existing Functionality)', () => {
        it('should handle single calendar ID as string', async () => {
            ...
            // Act
            const result = await listEventsHandler.runTool(
                args,
                mockOAuth2Client,
            );
    ...
    describe('Batch Request Creation', () => {
        it('should create proper batch requests for multiple calendars', () => {
            ...
            const expectedRequests = calendarIds.map((calendarId) => ({
                method: 'GET',
                path: `/calendar/v3/calendars/${encodeURIComponent(calendarId)}/events?${new URLSearchParams(
                    {
                        singleEvents: 'true',
                        orderBy: 'startTime',
            ...
            expect(expectedRequests[0].path).toContain(
                'calendars/primary/events',
            );
            expect(expectedRequests[1].path).toContain(
                'calendars/work%40example.com/events',
            );
            expect(expectedRequests[2].path).toContain(
                'calendars/personal%40example.com/events',
            );
        ...
        it('should handle optional parameters in batch requests', () => {
            ...
            expect(expectedRequest.path).toContain(
                'timeMin=2024-01-01T00%3A00%3A00Z',
            );
    ...
    describe('Error Handling', () => {
        it('should handle authentication errors', async () => {
            ...
            await expect(
                listEventsHandler.runTool(args, mockOAuth2Client),
...
describe('BatchRequestHandler', () => {
    ...
    it('should create proper multipart batch request body', () => {
        ...
        expect(expectedBody).toContain(
            'GET /calendar/v3/calendars/work%40example.com/events',
        );
    });
    ...
    it('should handle batch request with proper authorization header', async () => {
        const mockGetAccessToken = vi
            .fn()
            .mockResolvedValue({ token: 'mock_access_token' });
