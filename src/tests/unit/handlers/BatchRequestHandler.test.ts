...
import {
    ...
    BatchRequestHandler,
    BatchResponse,
} from '../../../handlers/core/BatchRequestHandler.js';
...
describe('BatchRequestHandler', () => {
    ...
    describe('Batch Request Creation', () => {
        ...
        it('should create proper multipart request body with multiple requests', () => {
            ...
            const boundaryCount = (
                result.match(new RegExp(`--${boundary}`, 'g')) || []
            ).length;
...
--batch_abc123--`;
            ...
            const responses = (batchHandler as any).parseBatchResponse(
                mockResponseText,
            );
...
--batch_abc123--`;
        ...
        it('should handle network errors during batch execution', async () => {
            ...
            await expect(noRetryHandler.executeBatch(requests)).rejects.toThrow(
                'Failed to execute batch request: Network error',
            );
        });

        it('should handle authentication errors', async () => {
            mockOAuth2Client.getAccessToken = vi
                .fn()
            ...
            await expect(batchHandler.executeBatch(requests)).rejects.toThrow(
