...
class RecurringEventHelpers {
        ...
        for (const rule of recurrence) {
            if (rule.startsWith('RRULE:')) {
                ...
                const updatedRule = `${rule
                    ...
                    .replace(/;COUNT=\d+/g, '')};UNTIL=${untilDate}`;
     ...
     */
    cleanEventForDuplication(
        event: calendar_v3.Schema$Event,
    ): calendar_v3.Schema$Event {
        const cleanedEvent = { ...event };

        // Remove fields that shouldn't be duplicated
        cleanedEvent.id = undefined;
        cleanedEvent.etag = undefined;
        cleanedEvent.iCalUID = undefined;
        cleanedEvent.created = undefined;
        cleanedEvent.updated = undefined;
        cleanedEvent.htmlLink = undefined;
        cleanedEvent.hangoutLink = undefined;
    ...
    buildUpdateRequestBody(args: any): calendar_v3.Schema$Event {
        ...
        if (timeChanged || (!args.start && !args.end && args.timeZone)) {
            ...
            if (!requestBody.start.timeZone)
                requestBody.start.timeZone = args.timeZone;
            if (!requestBody.end.timeZone)
                requestBody.end.timeZone = args.timeZone;
        }
...
describe('RecurringEventHelpers', () => {
    ...
    describe('detectEventType', () => {
        ...
        it('should handle API errors', async () => {
            ...
            await expect(
                helpers.detectEventType('invalid123', 'primary'),
    ...
    describe('formatInstanceId', () => {
        ...
        testCases.forEach(({ eventId, originalStartTime, expected }) => {
            it(`should format instance ID correctly for ${originalStartTime}`, () => {
                const result = helpers.formatInstanceId(
                    eventId,
                    originalStartTime,
                );
    ...
    describe('calculateEndTime', () => {
        it('should calculate end time based on original duration', () => {
            ...
            const result = helpers.calculateEndTime(
                newStartTime,
                originalEvent,
            );
    ...
    describe('updateRecurrenceWithUntil', () => {
        it('should add UNTIL clause to simple recurrence rule', () => {
            ...
            const result = helpers.updateRecurrenceWithUntil(
                recurrence,
                untilDate,
            );
        ...
        it('should replace existing UNTIL clause', () => {
            const recurrence = [
                'RRULE:FREQ=WEEKLY;BYDAY=MO;UNTIL=20240531T170000Z',
            ];
            const untilDate = '20240630T170000Z';
        ...
        it('should handle complex recurrence rules', () => {
            const recurrence = [
                'RRULE:FREQ=DAILY;INTERVAL=2;BYHOUR=10;BYMINUTE=0;COUNT=20',
            ];
            const untilDate = '20240630T170000Z';
            ...
            expect(result).toEqual([
                'RRULE:FREQ=DAILY;INTERVAL=2;BYHOUR=10;BYMINUTE=0;UNTIL=20240630T170000Z',
            ]);
        });

        it('should throw error for empty recurrence', () => {
            expect(() =>
                helpers.updateRecurrenceWithUntil([], '20240630T170000Z'),
            ).toThrow('No recurrence rule found');

            expect(() =>
                helpers.updateRecurrenceWithUntil(
                    undefined as any,
                    '20240630T170000Z',
                ),
            ).toThrow('No recurrence rule found');
        ...
        it('should throw error when no RRULE found', () => {
            ...
            expect(() =>
                helpers.updateRecurrenceWithUntil(recurrence, untilDate),
    ...
    describe('Edge Cases and Boundary Conditions', () => {
        it('should handle leap year dates correctly in formatInstanceId', () => {
            ...
            leapYearCases.forEach(
                ({ eventId, originalStartTime, expected }) => {
                    const result = helpers.formatInstanceId(
                        eventId,
                        originalStartTime,
                    );
        ...
        it('should handle calculateEndTime with very short and very long durations', () => {
            ...
            const shortResult = helpers.calculateEndTime(
                '2024-06-16T15:30:00Z',
                shortDurationEvent,
            );
            ...
            const longResult = helpers.calculateEndTime(
            ...
            const multiDayResult = helpers.calculateEndTime(
        ...
        it('should handle updateRecurrenceWithUntil with various RRULE formats', () => {
            const complexRRuleCases = [
                {
                    original: [
                        'RRULE:FREQ=MONTHLY;BYMONTHDAY=15;BYHOUR=10;BYMINUTE=30',
                    ],
                    untilDate: '20241215T103000Z',
                    expected: [
                        'RRULE:FREQ=MONTHLY;BYMONTHDAY=15;BYHOUR=10;BYMINUTE=30;UNTIL=20241215T103000Z',
                    ],
                },
                {
                    original: [
                        'RRULE:FREQ=YEARLY;BYMONTH=6;BYMONTHDAY=15;COUNT=5',
                    ],
                    untilDate: '20291215T103000Z',
                    expected: [
                        'RRULE:FREQ=YEARLY;BYMONTH=6;BYMONTHDAY=15;UNTIL=20291215T103000Z',
                    ],
                },
                {
                    original: [
                        'RRULE:FREQ=WEEKLY;BYDAY=MO,WE,FR;INTERVAL=2;UNTIL=20241201T100000Z',
                    ],
                    untilDate: '20241115T100000Z',
                    expected: [
                        'RRULE:FREQ=WEEKLY;BYDAY=MO,WE,FR;INTERVAL=2;UNTIL=20241115T100000Z',
                    ],
                },
            ];
        ...
        it('should handle cleanEventForDuplication with all possible system fields', () => {
            ...
            const result = helpers.cleanEventForDuplication(
                eventWithAllSystemFields,
            );
