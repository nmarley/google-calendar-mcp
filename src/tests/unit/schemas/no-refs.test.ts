...
describe('Schema $ref Prevention Tests', () => {
    it('should not generate $ref references in JSON schemas, causes issues with Claude Desktop', () => {
        ...
        for (const tool of tools) {
            ...
            if (hasRef) {
                console.error(
                    `Tool "${tool.name}" contains $ref in schema:`,
                    jsonSchema,
                );
            }
    ...
    it('should have unique schema instances for similar parameters', () => {
        ...
        for (const tool of tools) {
            ...
            if (
                schemaStr.includes('timeMin') ||
                schemaStr.includes('timeMax') ||
                schemaStr.includes('"start"') ||
                schemaStr.includes('"end"')
            ) {
    ...
    it('should detect if shared schema instances are reused', () => {
        ...
        const registryCode = require('node:fs').readFileSync(
            require('node:path').join(__dirname, '../../../tools/registry.ts'),
            'utf8',
        );
        ...
        for (const pattern of sharedSchemaUsage) {
            ...
            if (matches) {
                console.error(
                    `Found potentially problematic schema usage: ${matches[0]}`,
                );
