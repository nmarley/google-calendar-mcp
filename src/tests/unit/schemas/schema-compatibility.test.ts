...
describe('Schema Compatibility', () => {
        ...
        for (const tool of tools) {
            ...
            for (const feature of problematicFeatures) {
                if (schemaStr.includes(`"${feature}"`)) {
                    issues.push(
                        `Tool "${tool.name}" contains problematic feature: ${feature}`,
                    );
                }
            }
        }
    ...
    it('should validate specific known tool schemas', () => {
        ...
        if (listEventsSchema.properties) {
            ...
            const calendarIdStr = JSON.stringify(
                listEventsSchema.properties.calendarId,
            );
            ...
            console.warn(
                'MCP SDK not exposing full schema details for list-events tool',
            );
        }
    ...
    it('should test OpenAI schema conversion compatibility', () => {
        ...
        const validateOpenAISchema = (schema: any, toolName: string) => {
            if (schema.type !== 'object') {
                throw new Error(
                    `${toolName}: Schema must have type 'object' at top level, got '${schema.type}'`,
                );
            }
            ...
            for (const feature of problematicFeatures) {
                if (schemaStr.includes(`"${feature}"`)) {
                    throw new Error(
                        `${toolName}: Schema cannot contain '${feature}' at top level`,
                    );
                }
            }
        };
        ...
        for (const tool of tools) {
            ...
            expect(() =>
                validateOpenAISchema(openaiSchema, tool.name),
            ).not.toThrow();
        }
    });
    ...
    it('should validate array fields have proper items definition', () => {
        ...
        const toolsWithArrays = [
            'create-event',
            'update-event',
            'get-freebusy',
        ]; // These tools have array fields
...
describe('Schema Validation Rules Documentation', () => {
    it('should document MCP client compatibility requirements', () => {
        const rules = {
            ...
            'No oneOf/anyOf/allOf at top level':
                'These cause compatibility issues with OpenAI',
            'DateTime fields must have timezone':
                'RFC3339 format with timezone required',
            'Array fields must have items defined':
                'Proper validation requires items schema',
            'Enum fields must have type':
                'Type information required alongside enum values',
        };
