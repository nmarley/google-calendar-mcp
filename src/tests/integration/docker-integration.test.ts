...
describe('Docker Integration Tests', () => {
    ...
    describe('Docker Container Functionality', () => {
        ...
        google-calendar-mcp:test -c "while true; do sleep 30; done"`);
            ...
            console.log(
                `✅ Connected to MCP server in Docker container (${tools.tools.length} tools available)`,
            );
        ...
        google-calendar-mcp:test`);
            ...
            // Cleanup
            await execAsync(
                `docker stop ${CONTAINER_NAME}-http && docker rm ${CONTAINER_NAME}-http`,
            );
        }, 60000);
        ...
        it('should work with docker-compose', async () => {
            ...
            try {
                ...
                const { stdout: psStdio } = await execAsync(
                    'docker compose ps',
                    { cwd: process.cwd() },
                );
...
`;
    ...
    describe('Calendar Operations via Docker', () => {
        ...
        it('should list calendars through Docker', async () => {
            ...
            try {
                ...
                console.log(
                    `✅ Listed ${calendars.length} calendars via Docker`,
                );
            } catch (error) {
                testFactory.endTimer(
                    'docker-list-calendars',
                    startTime,
                    false,
                    String(error),
                );
        ...
        it('should create and manage events through Docker', async () => {
            ...
            const createStartTime = testFactory.startTimer(
                'docker-create-event',
            );
            ...
            try {
                ...
                const eventId =
                    TestDataFactory.extractEventIdFromResponse(createResult);
                ...
                const listStartTime =
                    testFactory.startTimer('docker-list-events');
                ...
                const events = Array.isArray(listResult.content)
                    ? listResult.content
                    : [listResult.content];
                const createdEvent = events.find((event: any) =>
                    event.text?.includes(eventData.summary),
                );
    ...
    describe('Performance and Resource Testing', () => {
        ...
        google-calendar-mcp:test -c "while true; do sleep 30; done"`);
            ...
            const memoryMB = Number.parseFloat(
                memUsage.split('/')[0].replace('MiB', '').trim(),
            );
            expect(memoryMB).toBeLessThan(200); // Should use less than 200MB

            // Parse CPU usage (e.g., "1.23%")
            const cpuPercent = Number.parseFloat(cpuUsage.replace('%', ''));
    ...
    async function cleanupDockerResources(): Promise<void> {
        const containerNames = [CONTAINER_NAME, `${CONTAINER_NAME}-http`];
        ...
        try {
            await execAsync(
                'docker volume rm mcp-test-tokens 2>/dev/null || true',
            );
        } catch (error) {
            // Ignore cleanup errors
        }
