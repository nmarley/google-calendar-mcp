...
export class TestDataFactory {
    private static readonly TEST_CALENDAR_ID =
        process.env.TEST_CALENDAR_ID || 'primary';
    ...
    static createAllDayEvent(overrides: Partial<TestEvent> = {}): TestEvent {
        ...
        return {
            ...
            start: TestDataFactory.formatDateTimeRFC3339(tomorrow),
            end: TestDataFactory.formatDateTimeRFC3339(dayAfter),
            timeZone: 'America/Los_Angeles',
            ...overrides,
        };
    }
    ...
    static createEventWithAttendees(
        ...
        if (!invitee1 || !invitee2) {
            throw new Error(
                'INVITEE_1 and INVITEE_2 environment variables are required for creating events with attendees',
            );
        }

        return TestDataFactory.createSingleEvent({
    ...
    static getTimeRanges() {
        ...
        return {
            // Past week
            pastWeek: {
                timeMin: TestDataFactory.formatDateTimeRFC3339WithTimezone(
                    new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000),
                ),
                timeMax: TestDataFactory.formatDateTimeRFC3339WithTimezone(now),
            },
            // Next week
            nextWeek: {
                timeMin: TestDataFactory.formatDateTimeRFC3339WithTimezone(now),
                timeMax: TestDataFactory.formatDateTimeRFC3339WithTimezone(
                    new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000),
                ),
            },
            // Next month
