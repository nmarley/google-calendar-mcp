...
class RealOpenAIMCPClient implements OpenAIMCPClient {
    ...
    async sendMessage(prompt: string): Promise<{
        ...
        if (!this.currentSessionId) {
            throw new Error(
                'No active test session. Call startTestSession() first.',
            );
        }
        ...
        try {
            ...
            const openaiTools = availableTools.tools.map((tool) => ({
                ...
                function: {
                    ...
                    parameters: this.convertMCPSchemaToOpenAISchema(
                        tool.inputSchema,
                    ),
                },
            }));
            ...
            for (const toolCall of toolCalls) {
                try {
                    const startTime = this.testFactory.startTimer(
                        `mcp-${toolCall.name}`,
                    );

                    console.log(
                        `🔧 Executing ${toolCall.name} with:`,
                    ...
                    if (toolCall.name === 'create-event') {
                        const eventId =
                            TestDataFactory.extractEventIdFromResponse(result);
                        if (eventId) {
                            this.testFactory.addCreatedEventId(eventId);
                            console.log(
                                `📝 Tracked created event ID: ${eventId}`,
                            );
                        }
                    }
                } catch (error) {
    ...
    private convertMCPSchemaToOpenAISchema(mcpSchema: any): any {
        ...
        const enhancedSchema = {
            type: 'object' as const,
            properties: this.enhancePropertiesForOpenAI(
        ...
        for (const [key, value] of Object.entries(properties)) {
            ...
            if (this.isDateTimeProperty(key, prop)) {
                enhanced[key] = {
                    ...
                    description:
                        `${prop.description || ''} CRITICAL: MUST be in RFC3339 format with timezone. Examples: "2024-01-01T10:00:00Z" (UTC) or "2024-01-01T10:00:00-08:00" (Pacific). NEVER use "2024-01-01T10:00:00" without timezone.`.trim(),
                };
            }

            // Recursively enhance nested objects
            if (prop.type === 'object' && prop.properties) {
                enhanced[key].properties = this.enhancePropertiesForOpenAI(
                    prop.properties,
                );
            }
    ...
    logInteractionContext(context: LLMInteractionContext): void {
        ...
        if (context.toolCalls.length > 0) {
            ...
            context.toolCalls.forEach((call, index) => {
                ...
                console.log(
                    `     Arguments: ${JSON.stringify(call.arguments, null, 4)}`,
                );
            });
            ...
            context.executedResults.forEach((result, index) => {
                console.log(
                    `  ${index + 1}. ${result.toolCall.name}: ${result.success ? '✅ SUCCESS' : '❌ FAILED'}`,
                );
                if (!result.success) {
                    console.log(
                        `     Error: ${JSON.stringify(result.result, null, 4)}`,
                    );
                } else {
                    console.log(
                        `     Result: ${JSON.stringify(result.result, null, 4)}`,
                    );
                }
            });
        }
...
describe('Complete OpenAI GPT + MCP Integration Tests', () => {
    ...
    beforeAll(async () => {
        console.log(
            '🚀 Starting complete OpenAI GPT + MCP integration tests...',
        );

        // Validate required environment variables
        if (!TEST_CALENDAR_ID) {
            throw new Error(
                'TEST_CALENDAR_ID environment variable is required',
            );
        }
        if (!INVITEE_1 || !INVITEE_2) {
            throw new Error(
                'INVITEE_1 and INVITEE_2 environment variables are required for testing event invitations',
            );
        }
        ...
        try {
            const testResponse = await openaiMCPClient.sendMessage(
                'Hello, can you list my calendars?',
            );
    ...
    afterAll(async () => {
        ...
        console.log(
            '🧹 Complete OpenAI GPT + MCP integration test cleanup completed',
        );
    }, 30000);
    ...
    describe('End-to-End Calendar Workflows', () => {
        it('should complete a full calendar management workflow', async () => {
            ...
            try {
                ...
                expect(
                    calendarsResponse.executedResults.length,
                ).toBeGreaterThan(0);
                ...
                if (
                    step1Context &&
                    openaiMCPClient instanceof RealOpenAIMCPClient
                ) {
                    console.log(
            ...
            try {
                ...
                expect(createResponse.executedResults.length).toBeGreaterThan(
                    0,
                );

                // Check if GPT eventually called create-event (may be after get-current-time or other tools)
                createToolCall = createResponse.executedResults.find(
                    (r) => r.toolCall.name === 'create-event',
                );
                ...
                if (createToolCall) {
                    ...
                    const timeToolCall = createResponse.executedResults.find(
                        (r) => r.toolCall.name === 'get-current-time',
                    );
                    if (timeToolCall) {
                        console.log(
                            '✅ Step 2: GPT gathered time information (reasonable first step)',
                        );

                        // Try a follow-up to complete the creation
                        const followUpResponse =
                            await openaiMCPClient.sendMessage(
                        ...
                        const followUpCreateResult =
                            followUpResponse.executedResults.find(
                                (r) => r.toolCall.name === 'create-event',
                            );
                        ...
                        if (followUpCreateResult?.success) {
                            ...
                            console.log(
                                '✅ Step 2: Created test event in follow-up',
                            );
                        } else {
                            // GPT understood but didn't complete creation - still valid
                            expect(
                                createResponse.content.toLowerCase(),
                            ).toMatch(/(meeting|event|created|tomorrow|test)/);
                            console.log(
                                '✅ Step 2: GPT understood request but did not complete creation',
                            );
                        }
                    } else {
                        console.log(
                            '⚠️ Step 2: GPT responded but did not call expected tools',
                        );
                        // Still consider this valid - GPT understood the request
                        expect(createResponse.content.toLowerCase()).toMatch(
                            /(meeting|event|created|tomorrow|test)/,
                        );
                    }
                }
            } catch (error) {
            ...
            if (createToolCall?.success) {
                ...
                const searchToolCall = searchResponse.executedResults.find(
                    (r) =>
                        r.toolCall.name === 'search-events' ||
                        r.toolCall.name === 'list-events',
                );
                ...
                console.log(
                    '⚠️ Step 3: Skipping search since no event was created',
                );
            }
        ...
        it('should handle event creation with complex details', async () => {
            ...
            await executeWithContextLogging(
                'Complex Event Creation',
                async () => {
                    const response = await openaiMCPClient.sendMessage(
                        `Create a team meeting called 'Weekly Standup with GPT' for next Monday at 9 AM, lasting 30 minutes. Add attendees ${INVITEE_1} and ${INVITEE_2}. Set it in Pacific timezone and add a reminder 15 minutes before.`,
                    );

                    expect(response.content).toBeDefined();
                    expect(response.executedResults.length).toBeGreaterThan(0);

                    const createToolCall = response.executedResults.find(
                        (r) => r.toolCall.name === 'create-event',
                    );
                    const timeResult = response.executedResults.find(
                        (r) => r.toolCall.name === 'get-current-time',
                    );
                    ...
                    if (createToolCall) {
                        ...
                        if (createToolCall?.toolCall.arguments.summary) {
                            expect(
                                createToolCall.toolCall.arguments.summary,
                            ).toContain('Weekly Standup');
                        }
                        if (createToolCall?.toolCall.arguments.attendees) {
                            expect(
                                createToolCall.toolCall.arguments.attendees
                                    .length,
                            ).toBe(2);
                        }
                        if (createToolCall?.toolCall.arguments.timeZone) {
                            expect(
                                createToolCall.toolCall.arguments.timeZone,
                            ).toMatch(/Pacific|America\/Los_Angeles/);
                        }
                        ...
                        console.log(
                            '🔄 GPT gathered time info first, attempting follow-up for complex event...',
                        );
...
Please use the create-event tool to create this event.`,
                        ...
                        if (followUpCreateResult?.success) {
                            ...
                            if (
                                followUpCreateResult?.toolCall.arguments.summary
                            ) {
                                expect(
                                    followUpCreateResult.toolCall.arguments
                                        .summary,
                                ).toContain('Weekly Standup');
                            }
                            if (
                            ...
                            console.log(
                                '✅ Complex event creation successful in follow-up',
                            );
                        } else {
                            // GPT understood but didn't complete creation - still valid
                            expect(response.content.toLowerCase()).toMatch(
                                /(meeting|standup|monday|team)/,
                            );
                            console.log(
                                '✅ Complex event creation: GPT understood request',
                            );
                        }
                    } else {
                        ...
                        console.log(
                            '✅ Complex event creation: GPT provided reasonable response',
                        );
                    }
        ...
        it('should handle availability checking and smart scheduling', async () => {
            ...
            try {
                ...
                const availabilityCheck = response.executedResults.find(
                    (r) =>
                        r.toolCall.name === 'get-freebusy' ||
                        r.toolCall.name === 'list-events' ||
                        r.toolCall.name === 'get-current-time',
                );
...
Call the create-event tool now with these exact values.`,
                ...
                const createResult = createResponse.executedResults.find(
                    (r) => r.toolCall.name === 'create-event',
                );
                const timeResult = createResponse.executedResults.find(
                    (r) => r.toolCall.name === 'get-current-time',
                );
                ...
                if (createResult) {
                    ...
                    if (!createResult.success) {
                        console.log(
                            '❌ Event creation failed, skipping modification test',
                        );
                        console.log(
                            'Error:',
                    ...
                    eventId = TestDataFactory.extractEventIdFromResponse(
                        createResult.result,
                    );
                    if (!eventId) {
                        console.log(
                            '❌ Could not extract event ID from creation result, skipping modification test',
                        );
                        return;
                    }
                    ...
                    console.log(
                        '🔄 GPT gathered time info first, attempting follow-up to complete creation...',
                    );
...
Please use the create-event tool to actually create this event now.`,
                    ...
                    if (!followUpCreateResult) {
                        console.log(
                            'GPT did not complete event creation in follow-up, trying one more approach...',
                        );

                        // Try a third approach with even more explicit instructions
                        const finalAttemptResponse =
                            await openaiMCPClient.sendMessage(
                                "Please call the create-event tool now to create a meeting titled 'Test Event for Modification' for tomorrow at 2 PM.",
                            );

                        const finalCreateResult =
                            finalAttemptResponse.executedResults.find(
                                (r) => r.toolCall.name === 'create-event',
                            );

                        if (!finalCreateResult) {
                            console.log(
                                'GPT did not create event after multiple attempts, skipping modification test',
                            );
                            return;
                        }
                        ...
                        if (!finalCreateResult.success) {
                            console.log(
                                '❌ Event creation failed in final attempt, skipping modification test',
                            );
                            console.log(
                                'Error:',
                                JSON.stringify(
                                    finalCreateResult.result,
                                    null,
                                    2,
                                ),
                            );
                        ...
                        if (!eventId) {
                            console.log(
                                '❌ Could not extract event ID from final creation result, skipping modification test',
                            );
                            return;
                        }
                        ...
                        if (!followUpCreateResult.success) {
                            console.log(
                                '❌ Event creation failed in follow-up, skipping modification test',
                            );
                            console.log(
                                'Error:',
                                JSON.stringify(
                                    followUpCreateResult.result,
                                    null,
                                    2,
                                ),
                            );
                        ...
                        if (!eventId) {
                            console.log(
                                '❌ Could not extract event ID from follow-up creation result, skipping modification test',
                            );
                            return;
                        }
                        console.log(
                            '✅ Event created in follow-up interaction',
                        );
                    }
                } else {
                    console.log(
                        'GPT did not call create-event or get-current-time, skipping modification test',
                    );
                    return;
                }
...
Call the update-event tool now with these exact values to update the event.`,
                ...
                const updateResult = modifyResponse.executedResults.find(
                    (r) => r.toolCall.name === 'update-event',
                );
                ...
                if (updateResult) {
                    ...
                    console.log(
                        '🔄 GPT responded with guidance, trying more direct approach...',
                    );
                    ...
                    if (modifyResponse.context) {
                        console.log(
                            '🔧 Available tools:',
                            modifyResponse.context.availableTools.join(', '),
                        );
                    }

                    const directUpdateResponse =
                        await openaiMCPClient.sendMessage(
...
Execute the update-event tool call immediately.`,
                    ...
                    const directUpdateResult =
                        directUpdateResponse.executedResults.find(
                            (r) => r.toolCall.name === 'update-event',
                        );
                    ...
                    if (directUpdateResult) {
                        ...
                        console.log(
                            '✅ Event modification successful in follow-up',
                        );
                    } else {
                        // GPT understood but didn't use tools - still valid
                        expect(modifyResponse.content.toLowerCase()).toMatch(
                            /(update|modify|change|move|title|modified|event|calendar)/,
                        );
                        console.log(
                            '✅ Event modification: GPT understood request but provided guidance instead of using tools',
                        );
                    }
                } else {
                    // GPT made other tool calls but not update-event
                    expect(modifyResponse.content.toLowerCase()).toMatch(
                        /(update|modify|change|move|title|modified)/,
                    );
                    console.log(
                        '✅ Event modification: GPT understood request but did not call update-event tool',
                    );
                }
    ...
    describe('Natural Language Understanding with Real Execution', () => {
        it('should understand and execute various time expressions', async () => {
            openaiMCPClient.startTestSession(
                'Time Expression Understanding Test',
            );
                        ...
                        async () => {
                            ...
                            if (createResult) {
                                ...
                                if (createResult?.toolCall.arguments.start) {
                                    expect(
                                        createResult.toolCall.arguments.start,
                                    ).toBeDefined();
                                }
                                if (createResult?.toolCall.arguments.end) {
                                    expect(
                                        createResult.toolCall.arguments.end,
                                    ).toBeDefined();
                                }

                                console.log(
                                    `✅ Time expression "${timeExpr}" created successfully`,
                                );
                            } else if (timeResult?.success) {
                                // GPT gathered time info first, try a follow-up to complete creation
                                console.log(
                                    `🔄 Time expression "${timeExpr}" - GPT gathered timing info first, attempting follow-up...`,
                                );
                                ...
                                const followUpCreateResult =
                                    followUpResponse.executedResults.find(
                                        (r) =>
                                            r.toolCall.name === 'create-event',
                                    );

                                if (followUpCreateResult) {
                                    expect(followUpCreateResult.success).toBe(
                                        true,
                                    );
                                    console.log(
                                        `✅ Time expression "${timeExpr}" created successfully in follow-up`,
                                    );
                                } else {
                                    // GPT understood but didn't call expected tools - still valid if response is reasonable
                                    expect(
                                        followUpResponse.content.toLowerCase(),
                                    ).toMatch(
                                        /(meeting|event|time|tomorrow|friday|days)/,
                                    );
                                    console.log(
                                        `✅ Time expression "${timeExpr}" - GPT provided reasonable response in follow-up`,
                                    );
                                }
                            } else {
                                // GPT understood but didn't call expected tools - still valid if response is reasonable
                                ...
                                console.log(
                                    `✅ Time expression "${timeExpr}" - GPT provided reasonable response`,
                                );
                            }
                        },
                    );
        ...
        it('should handle complex multi-step requests', async () => {
            ...
            try {
                ...
                const listEventsCall = response.executedResults.find(
                    (r) => r.toolCall.name === 'list-events',
                );
                const createEventCall = response.executedResults.find(
                    (r) => r.toolCall.name === 'create-event',
                );
                const searchEventsCall = response.executedResults.find(
                    (r) => r.toolCall.name === 'search-events',
                );

                expect(
                    listEventsCall || createEventCall || searchEventsCall,
                ).toBeDefined();
    ...
    describe('Error Handling and Edge Cases', () => {
        it('should gracefully handle invalid requests', async () => {
            ...
            try {
                ...
                expect(response.content.toLowerCase()).toMatch(
                    /(cannot|invalid|past|error|sorry|issue|valid)/,
                );
                ...
                console.error(
                    '❌ Invalid request handling test failed:',
                    error,
                );
                openaiMCPClient.endTestSession();
                throw error;
    ...
    async function executeWithContextLogging<T>(
        ...
        try {
            ...
            if (lastContext) {
                ...
                (openaiMCPClient as RealOpenAIMCPClient).logInteractionContext(
                    lastContext,
                );
            }
            throw error;
    ...
    async function cleanupEvents(eventIds: string[]): Promise<void> {
        ...
        for (const eventId of eventIds) {
            try {
                ...
                console.warn(
                    `Failed to cleanup event ${eventId}:`,
                    String(error),
                );
            }
        }
