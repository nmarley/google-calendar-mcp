...
describe('Complete Claude Haiku + MCP Integration Tests', () => {
    ...
    beforeAll(async () => {
        ...
        const testResponse = await claudeMCPClient.sendMessage(
    ...
    afterAll(async () => {
        ...
        console.log(
            '🧹 Complete Claude + MCP integration test cleanup completed',
        );
    }, 30000);
    ...
    describe('End-to-End Calendar Workflows', () => {
        it('should complete a full calendar management workflow', async () => {
            ...
            try {
                ...
                if (
                    step1Context &&
                    claudeMCPClient instanceof RealClaudeMCPClient
                ) {
                    console.log(
            ...
            try {
                ...
                if (createToolCall) {
                    ...
                    if (timeToolCall) {
                        console.log(
                            '✅ Step 2: Claude gathered time information (reasonable first step)',
                        );
                    } else {
                        console.log(
                            '⚠️ Step 2: Claude responded but did not call expected tools',
                        );
                    }
            ...
            await executeWithContextLogging(
                ...
                async () => {
                    const response = await claudeMCPClient.sendMessage(
                        `Create a team meeting called 'Weekly Standup with Claude' for next Monday at 9 AM, lasting 30 minutes. Add attendees ${INVITEE_1} and ${INVITEE_2}. Set it in Pacific timezone and add a reminder 15 minutes before.`,
                    );
                    ...
                    if (createToolCall) {
                        ...
                        console.log(
                            '🔄 Claude gathered time info first, attempting follow-up for complex event...',
                        );

                        const followUpResponse =
                            await claudeMCPClient.sendMessage(
...
Please use the create-event tool to create this event.`,
                        ...
                        if (followUpCreateResult?.success) {
                            ...
                            console.log(
                                '✅ Complex event creation: Claude understood request',
                            );
                        }
                    } else {
                        ...
                        console.log(
                            '✅ Complex event creation: Claude provided reasonable response',
                        );
                    }
                },
            );
...
Call the create-event tool now with these exact values.`,
                ...
                if (createResult) {
                    ...
                    console.log(
                        '🔄 Claude gathered time info first, attempting follow-up to complete creation...',
                    );
...
Please use the create-event tool to actually create this event now.`,
                    ...
                    if (!followUpCreateResult) {
                        console.log(
                            'Claude did not complete event creation in follow-up, trying one more approach...',
                        );
                        ...
                        if (!finalCreateResult) {
                            console.log(
                                'Claude did not create event after multiple attempts, skipping modification test',
                            );
                            return;
                        }
                    ...
                    console.log(
                        'Claude did not call create-event or get-current-time, skipping modification test',
                    );
                    return;
                }
...
Call the update-event tool now with these exact values to update the event.`,
                ...
                if (updateResult) {
                    ...
                    console.log(
                        '🔄 Claude responded with guidance, trying more direct approach...',
                    );
...
Execute the update-event tool call immediately.`,
                    ...
                    if (directUpdateResult) {
                        ...
                        console.log(
                            '✅ Event modification: Claude understood request but provided guidance instead of using tools',
                        );
                    }
                } else {
                    ...
                    console.log(
                        '✅ Event modification: Claude understood request but did not call update-event tool',
                    );
                }
            });
    ...
    describe('Natural Language Understanding with Real Execution', () => {
            ...
            for (const timeExpr of timeExpressions) {
                await executeWithContextLogging(
                    ...
                    async () => {
                        ...
                        expect(response.content).toBeDefined();
                        expect(response.executedResults.length).toBeGreaterThan(
                            0,
                        );
                        ...
                        if (createResult) {
                            ...
                            console.log(
                                `🔄 Time expression "${timeExpr}" - Claude gathered timing info first, attempting follow-up...`,
                            );
                            ...
                            if (followUpCreateResult) {
                                ...
                                console.log(
                                    `✅ Time expression "${timeExpr}" - Claude provided reasonable response in follow-up`,
                                );
                            }
                        } else {
                            // Claude understood but didn't call expected tools - still valid if response is reasonable
                            ...
                            console.log(
                                `✅ Time expression "${timeExpr}" - Claude provided reasonable response`,
                            );
                        }
                    },
                );
        ...
        it('should handle complex multi-step requests', async () => {
            ...
            const getCurrentTimeCall = response.executedResults.find(
                (r) => r.toolCall.name === 'get-current-time',
            );
            const getFreeBusyCall = response.executedResults.find(
                (r) => r.toolCall.name === 'get-freebusy',
            );

            // Accept any calendar-related tool call as evidence Claude understood the complex request
            const anyCalendarAction =
                listEventsCall ||
                createEventCall ||
                searchEventsCall ||
                getCurrentTimeCall ||
            ...
            if (anyCalendarAction) {
                ...
                const actions = response.executedResults
                    .map((r) => r.toolCall.name)
                    .join(', ');
                console.log(
                    `✅ Multi-step request: Claude executed [${actions}]`,
                );
            } else {
                // If no tools called, at least verify Claude understood the request
                expect(response.content.toLowerCase()).toMatch(
                    /(calendar|week|tuesday|meeting|schedule|available)/,
                );
                console.log(
                    '✅ Multi-step request: Claude understood but chose not to use tools',
                );
            }
        ...
        it('should gracefully handle invalid requests', async () => {
            await executeWithContextLogging(
                ...
                async () => {
                    ...
                    expect(response.content.toLowerCase()).toMatch(
                        /(cannot|invalid|past|error|sorry)/,
                    );
    ...
    async function executeWithContextLogging<T>(
        ...
        try {
            ...
            if (lastContext) {
                ...
                (claudeMCPClient as RealClaudeMCPClient).logInteractionContext(
                    lastContext,
                );
            }
            throw error;
