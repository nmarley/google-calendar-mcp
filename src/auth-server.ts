...
async function runAuthServer() {
    ...
    try {
        ...
        process.stderr.write(
            `Waiting for OAuth callback on port ${authServer.getRunningPort()}...\n`,
        );
        ...
        const pollInterval = setInterval(async () => {
            try {
                if (authServer?.authCompletedSuccessfully) {
                    process.stderr.write(
                        'Authentication completed successfully detected. Stopping server...\n',
                    );
                    clearInterval(pollInterval);
                    await authServer.stop();
                    process.stderr.write(
                        'Authentication successful. Server stopped.\n',
                    );
                    process.exit(0);
                } else {
                ...
                process.stderr.write(
                    `Error in polling interval: ${error instanceof Error ? error.message : 'Unknown error'}\n`,
                );
                clearInterval(pollInterval);
