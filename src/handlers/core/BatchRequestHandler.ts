...
export class BatchRequestHandler {
    private readonly batchEndpoint =
    ...
    async executeBatch(requests: BatchRequest[]): Promise<BatchResponse[]> {
        ...
        if (requests.length > 50) {
            throw new Error(
                'Batch requests cannot exceed 50 requests per batch',
            );
        }
    ...
    private async executeBatchWithRetry(
        ...
        try {
            ...
            if (attempt < this.maxRetries && this.isRetryableError(error)) {
                ...
                process.stderr.write(
                    `Network error, retrying after ${delay}ms (attempt ${attempt + 1}/${this.maxRetries}): ${error instanceof Error ? error.message : 'Unknown error'}\n`,
                );
                await this.sleep(delay);
                return this.executeBatchWithRetry(requests, attempt + 1);
            }
