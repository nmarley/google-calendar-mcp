...
export class ListEventsHandler extends BaseToolHandler {
    async runTool(
        ...
        if (calendarIds.length === 1) {
            ...
            allEvents.forEach((event, index) => {
                const eventDetails = formatEventWithDetails(
                    event,
                    event.calendarId,
                );
    ...
    private async fetchEvents(
        ...
        if (calendarIds.length === 1) {
            return this.fetchSingleCalendarEvents(
                client,
                calendarIds[0],
                options,
            );
    ...
    private async fetchSingleCalendarEvents(
        ...
        try {
            ...
            if (timeMin || timeMax) {
                const timezone =
                    options.timeZone ||
                    (await this.getCalendarTimezone(client, calendarId));
                timeMin = timeMin
                    ? convertToRFC3339(timeMin, timezone)
                    : undefined;
                timeMax = timeMax
                    ? convertToRFC3339(timeMax, timezone)
                    : undefined;
            }
    ...
    private async fetchMultipleCalendarEvents(
        ...
        const { events, errors } = this.processBatchResponses(
            responses,
            calendarIds,
        );
        ...
        if (errors.length > 0) {
            process.stderr.write(
                `Some calendars had errors: ${errors.map((e) => `${e.calendarId}: ${e.error}`).join(', ')}\n`,
            );
        }
