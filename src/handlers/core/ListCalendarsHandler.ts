        ...
        return calendars
            .map((cal) => {
                ...
                const name = this.sanitizeString(
                    cal.summaryOverride || cal.summary || 'Untitled',
                );
                ...
                const accessRole = this.sanitizeString(
                    cal.accessRole || 'Unknown',
                );
                const isPrimary = cal.primary ? ' (PRIMARY)' : '';
                const isSelected = cal.selected !== false ? 'Yes' : 'No';
                const isHidden = cal.hidden ? 'Yes' : 'No';
                const backgroundColor = this.sanitizeString(
                    cal.backgroundColor || 'Default',
                );
                ...
                if (cal.defaultReminders && cal.defaultReminders.length > 0) {
                    defaultReminders = cal.defaultReminders
                        .map((reminder) => {
                            const method = this.sanitizeString(
                                reminder.method || 'unknown',
                            );
