...
export class GetCurrentTimeHandler extends BaseToolHandler {
            ...
            result = {
                currentTime: {
                    ...
                    requestedTimeZone: {
                        ...
                        rfc3339: this.formatDateInTimeZone(
                            now,
                            requestedTimeZone,
                        ),
                        humanReadable: this.formatHumanReadable(
                            now,
                            requestedTimeZone,
                        ),
    ...
    private getTimezoneOffsetMinutes(timeZone: string): number {
        ...
        const targetTime = new Date(
            `${targetTimeString.replace(' ', 'T')}Z`,
        ).getTime();
        const utcTimeParsed = new Date(
